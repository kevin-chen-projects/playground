"""
Syntax warm-up drills — PRACTICE FILE.

~30 tiny "write one idiom" exercises. Each function has a docstring telling you
exactly what to return, a blanked-out body, and an assert that checks your answer.

How to use:
    1. Fill in each function body (replace the TODO / `pass`).
    2. Run:  python syntax_drills.py
    3. It stops at the first wrong/blank answer. Fix, rerun, repeat until it
       prints "all 30 drills pass".

Goal: rebuild reflexes for slicing, comprehensions, dict/set methods, sorting,
collections, dataclasses, and copy — the stuff the IMC test assumes you know cold.
Check yourself against solutions_syntax_drills.py only after you've tried.
"""
...
def d27_top_two(items):
    """Return the two most common items as a list of (item, count), most common
    first. Use Counter."""
    pass  # TODO
...
def d28_group_by_first_letter(words):
    ...
    (input order preserved). Use defaultdict."""
    pass  # TODO
...
def d29_make_point(x, y):
    """Define a dataclass Point with fields x, y inside this function, and
    return Point(x, y). The returned object must have .x and .y attributes."""
    pass  # TODO
...
def d30_safe_snapshot(state):
    """state is a dict whose values are lists. Return a snapshot such that
    later mutating state's inner lists does NOT change the snapshot."""
    pass  # TODO
